...
exports.calculateOvertime = exports.processOvertimeJson = void 0;
exports.processPunchRecords = processPunchRecords;
// 预定义地区配置
 ...
 */
const processOvertimeJson = async (overtimeData) => {
    try {
        // 提取重要参数
...
exports.processOvertimeJson = processOvertimeJson;
/**
 * 根据地区获取在岗时长配置
 *
 * @param region 地区名称
 ...
 * @returns 处理结果
 */
function processCustomOvertimeData(customData, hourlyRate, overtimeStartTime = "19:00", region) {
    const result = [];
    const dailyRecords = []; // 存储每日详细记录
    ...
    for (const item of customData) {
        ...
        // 计算详细的工时信息
        const detailedCalculation = calculateDetailedWorkHours(firstCheckTime, lastCheckTime, dayType, overtimeStartTime, region);
        // 计算加班薪资（旷工时间会导致负薪资）
 ...
 */
async function intelligentDayType(date) {
    try {
        // 首先验证日期格式
 ...
 * @returns 详细工时计算结果
 */
function calculateDetailedWorkHours(firstCheckTime, lastCheckTime, dayType, overtimeStartTime = "19:00", region) {
    const notes = [];
    // 设置默认午休时长
    const lunchBreakHours = 1.5; // 固定1.5小时午休
    // 检查是否为异地打卡
 ...
 * 计算工作日详细信息
 */
function calculateWorkdayDetails(firstTime, lastTime, firstCheckTime, lastCheckTime, overtimeStartTime, region, notes) {
 ...
 * 计算周末和节假日详细信息
 */
function calculateWeekendHolidayDetails(firstTime, lastTime, firstCheckTime, lastCheckTime, dayType, notes) {
    const firstTimeMinutes = firstTime.hours * 60 + firstTime.minutes;
    const lastTimeMinutes = lastTime.hours * 60 + lastTime.minutes;
 ...
 * 输出每日汇总日志
 */
function logDailySummary(dailyRecords) {
    console.log('\n=== 每日考勤与加班汇总 ===');
    let totalWorkdays = 0;
    ...
    dailyRecords.forEach(record => {
        ...
        if (record.isUnderwork || record.deductionHours > 0 || record.attendanceStatus !== 'normal') {
            ...
            if (record.lateMinutes > 0)
                console.log(`  迟到: ${record.lateMinutes}分钟`);
            if (record.earlyLeaveMinutes > 0)
                console.log(`  早退: ${record.earlyLeaveMinutes}分钟`);
            if (record.deductionHours > 0)
                console.log(`  扣除工时: ${record.deductionHours.toFixed(2)}小时 (${record.deductionReason})`);
 ...
 * @returns 加班费率
 */
function calculatePayRate(dayType) {
 ...
 * @returns 加班薪资
 */
function calculateOvertimePay(overtime, rate, hourlyRate) {
 ...
 * @returns 餐补金额
 */
function calculateAllowance(overtime, dayType) {
 ...
 * @returns 迟到时间（分钟）
 */
function calculateLateTime(firstCheckTime, dayType) {
    // 非工作日或异地打卡不计算迟到
 ...
 * @returns 应扣除的工时（小时）
 */
function calculateLateDeduction(firstCheckTime, dayType) {
    // 非工作日或异地打卡不扣除工时
    if (dayType !== '工作日' || firstCheckTime.includes('异地打卡')) {
        return 0;
    }
 ...
 * @returns 汇总结果
 */
function summarizeResults(result, personalLeaveHours, sickLeaveHours, hourlyRate) {
    // 初始化汇总数据
    let totalOvertimePay = 0;
 ...
 * @returns Promise<FormattedSummaryResult> 返回加班工资计算结果
 */
async function processPunchRecords(punchRecords, hourlyRate = 20, workdayType, overtimeStartTime) {
    try {
        ...
        return await (0, exports.processOvertimeJson)(overtimeData);
    }
    catch (error) {
 ...
 * @returns 按日期分组的打卡记录
 */
function groupPunchRecordsByDate(punchRecords) {
    const recordsByDate = {};
    // 添加所有打卡记录
 ...
 * @returns 标准格式的加班记录
 */
async function convertToOvertimeRecords(recordsByDate, workdayType) {
 ...
 */
const calculateOvertime = async (hoursWorked, hourlyRate, cookie, yearMonth) => {
    try {
