...
const tcpRoutes_1 = __importDefault(require("@routes/tcpRoutes"));
const udpRoutes_1 = __importDefault(require("@routes/udpRoutes"));
const mqttRoutes_1 = __importDefault(require("@routes/mqttRoutes"));
const apiDocRoutes_1 = __importDefault(require("@routes/apiDocRoutes"));
const errorHandler_1 = require("@middlewares/errorHandler");
const fileRoutes_1 = __importDefault(require("@routes/fileRoutes"));
const postgresRoutes_1 = __importDefault(require("@routes/postgresRoutes"));
...
app.use('/api/http', httpRoutes_1.default);
app.use('/api/tcp', tcpRoutes_1.default);
app.use('/api/udp', udpRoutes_1.default);
app.use('/api/mqtt', mqttRoutes_1.default);
app.use('/api/files', fileRoutes_1.default);
app.use('/api/postgres', postgresRoutes_1.default);
app.use('/api/messages', messageRoutes_1.default);
app.use('/api/docs', apiDocRoutes_1.default);
// 专门处理 favicon.ico 路由
app.get('/favicon.ico', (req, res) => {
    const faviconPath = isProduction
        ? path_1.default.join(__dirname, '..', 'favicon.ico') // dist/favicon.ico
        : path_1.default.join(__dirname, '..', 'public', 'favicon.ico'); // public/favicon.ico
    res.set({
...
app.get('/api/docs', (req, res) => {
    res.json({
        success: true,
        data: (0, apiDocService_1.getApiDocs)(),
        message: 'API documentation retrieved successfully'
    });
});
// MQTT 服务
mqttService_1.mqttService.isSerListening(); // 检查MQTT服务是否在监听
// TCP 服务
tcpService_1.tcpService.isSerListening(); // 检查TCP服务是否在监听
// UDP 服务
udpService_1.udpService.isSerListening(); // 检查UDP服务是否在监听
// 静态文件路径 - 根据环境确定路径
...
if (isProduction) {
    ...
    publicPath = path_1.default.join(__dirname, '..', 'public'); // dist/public
    tempsPath = path_1.default.join(__dirname, 'temps'); // dist/server/temps
}
else {
    // 开发环境：server/server.ts，所以 __dirname 是 server
    publicPath = path_1.default.join(__dirname, 'public'); // server/public
    tempsPath = path_1.default.join(__dirname, 'temps'); // server/temps
}
// 确保目录存在（可选）
const fs_1 = __importDefault(require("fs"));
if (!fs_1.default.existsSync(publicPath))
    fs_1.default.mkdirSync(publicPath, { recursive: true });
if (!fs_1.default.existsSync(tempsPath))
    fs_1.default.mkdirSync(tempsPath, { recursive: true });
// Vue 前端静态文件服务 (生产环境)
if (isProduction) {
    ...
    // temps 目录服务
    app.use("/temps", express_1.default.static(tempsPath));
...
app.get("/script.js", (req, res) => {
    const scriptPath = isProduction
        ? path_1.default.join(__dirname, "script.js")
        : path_1.default.join(__dirname, "public", "script.js");
...
(0, webdavService_1.setupWebDAV)(app); // 设置 WebDAV
// 错误处理
